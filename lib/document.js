...
this.mount = function(app) {
    ...
    app.get('/:db/_all_docs', function(req, res) {
        _doView(req, res, function(opts, cb) {
            return Couch.dbAllDocs(req.params.db, opts, cb);
        });
    });

    app.post('/:db/_all_docs', function(req, res) {
        _doView(req, res, function(_, cb) {
            return Couch.dbGetDocs(req.params.db, req.json, cb);
        });
    });

    app.post('/:db/_temp_view', function(req, res) {
        _doView(req, res, function(opts, cb) {
            return Couch.dbTempView(req.params.db, req.json, opts, cb);
        });
    });

    app.get('/:db/_design/:design/_view/:view', function(req, res) {
        _doView(req, res, function(opts, cb) {
            return Couch.dbView(req.params.db, req.params.design, req.params.view, opts, cb);
        });
    });
    
    app.post('/:db/_bulk_docs', function(req, res) {
        var json = Couch.dbBulkDocs(req.params.db, req.json.docs);
    ...
    app.get('/:db/:doc_id/:child?', function(req, res) {
        var json = Couch.dbGetDoc(req.params.db, _doc_id(req.params), req.query);
    ...
    app.put('/:db/:doc_id/:child?', function(req, res) {
        var json = Couch.dbPutDoc(req.params.db, _doc_id(req.params), req.json);
    ...
    app.post('/:db', function(req, res) {
        var json = Couch.dbPutDoc(req.params.db, undefined, req.json);
        if (req.query.batch === 'ok') {
            res.json({ ok: true }, 202);
            return;
        }
    ...
    app.del('/:db/:doc_id/:child?', function(req, res) {
        var json = Couch.dbDeleteDoc(req.params.db, _doc_id(req.params), req.query.rev);
